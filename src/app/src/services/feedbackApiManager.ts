...
export const getFeedbackIds = async (): Promise<string[]> => {
    const response = await axios.get(`${_BASE_URL}/image_feedback/feedback/documents?select=id`);
    const data = response.data as { id: string }[];
    return data.map((item) => item.id);
};

export const getFeedback = async (id: string) => {
    const response = await axios.get(`${_BASE_URL}/image_feedback/feedback/documents/${id}`);
...
export const uploadFeedback = async (sessionId: string, id: string, feedback: string, rating: string) => {
    ...
    await axios.post(`${_BASE_URL}/image_feedback/feedback/documents/${id}/feedback`, { feedback, rating }, { headers: headers });
};
